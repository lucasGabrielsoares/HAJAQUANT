...
export function correlation(a,b){const n=Math.min(a.length,b.length);if(n<2)return null;const x=a.slice(-n),y=b.slice(-n),ax=x.reduce((s,v)=>s+v,0)/n,ay=y.reduce((s,v)=>s+v,0)/n;let num=0,dx=0,dy=0;for(let i=0;i<n;i++){const u=x[i]-ax,v=y[i]-ay;num+=u*v;dx+=u*u;dy+=v*v}return dx&&dy?num/Math.sqrt(dx*dy):null}
export{bestPortfolio,optimizePortfolio,runPortfolioMonteCarlo};
